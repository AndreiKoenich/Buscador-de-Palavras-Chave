...
Em seguida, realiza-se o processo de indexacao. O programa armazena cada palavra de cada tweet em um no de uma arvore ABP, sendo que
...
contidas na arvore ABP criada na indexacao, e imprime em um arquivo texto de saida a palavra em si e os tweets em que ela ocorre.

O numero de operacoes de comparacao que sao executadas ao longo do programa sao contabilizados e tambem impressos
no arquivo texto de saida, assim como o numero de nos e a altura da arvore ABP criada durante a indexacao. */
...
int comparacoes = 0; /* Variavel global para contabilizar a quantidade comparacoes realizadas ao longo do programa. */
...
pNodoA* InsereABP (pNodoA *a, TipoInfoAux chave) /* Funcao para inserir nos na ABP de acordo com a ordem lexicografica das palavras. */
{
    comparacoes++; /* Incrementacao do numero de comparacoes. */

    if (a == NULL) /* Ao chegar em um ponteiro nulo, realiza-se a insercao. */
    {
        ...
        a->info.ptListaTweet = insere_inicio_lista(a->info.ptListaTweet, chave.numero); /* Insere um numero de tweet no fim da lista. */
        strcpy (a->info.palavra, chave.word); /* Insere uma palavra no nodo. */
...
pNodoA* indexa_arvoreABP (pNodoA *a, char *arquivo_entrada) /* Funcao para realizar as indexacoes das palavras e dos tweets em uma arvore binaria de pesquisa. */
{
...
void consulta_arvoreABP (pNodoA *a, char *arquivo_consultas, char *arquivo_saida)
{
    ...
    if (!(arq1 = fopen (arquivo_consultas, "r"))) /* Abertura do arquivo texto de consultas para leitura, com controle dos casos em que nao e possivel abri-lo. */
    {
        system("cls");
...
int main ()
{
    ...
    printf("BUSCADOR DE PALAVRAS-CHAVE COM ARVORE ABP\n");
    ...
    pNodoA *raiz = inicializa_arvore(); /* Inicializa uma arvore binaria de pesquisa. */
    raiz = indexa_arvoreABP (raiz, arquivo_entrada); /* Realiza a indexacao das palavras e dos tweets do arquivo de entrada na arvore binaria de pesquisa. */
    ...
    consulta_arvoreABP (raiz, arquivo_consultas, arquivo_saida); /* Realiza a consulta das palavras do arquivo de consultas na arvore e as imprime no arquivo de saida, com seus respectivos tweets. */
